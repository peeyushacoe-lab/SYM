...
export async function POST(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  const db = getDb();
  const exam = (await db.prepare('SELECT * FROM exams WHERE id = ? AND school_id = ?').get(params.id, auth.session.schoolId)) as any;
  ...
  const upsert = db.prepare(
    `INSERT INTO exam_marks (exam_id, student_id, marks, remarks, school_id) VALUES (?, ?, ?, ?, ?)
  ...
  const tx = db.transaction(async (rows: any[]) => {
    for (const r of rows) {
      ...
      if (r.marks === null || r.marks === undefined || r.marks === '') {
        ...
        await upsert.run(params.id, r.student_id, Number(r.marks), r.remarks || null, auth.session.schoolId);
      }
    }
  });
  await tx(records);
