  ...
  const students = db
    .prepare(
      `SELECT s.id, s.name, s.roll_number, m.marks, m.remarks
       ...
       WHERE s.batch_id = ? ORDER BY CAST(s.roll_number AS INTEGER), s.name`
    )
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  const result = await authorize(params.id);
  if ('error' in result) return result.error;
  result.db.prepare('DELETE FROM exam_marks WHERE exam_id = ?').run(params.id);
  result.db.prepare('DELETE FROM exams WHERE id = ?').run(params.id);
  return NextResponse.json({ ok: true });
}
