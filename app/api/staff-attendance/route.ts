...
export async function GET(req: NextRequest) {
  ...
  const db = getDb();
  const staff = await db.prepare('SELECT id, name, designation FROM staff ORDER BY name').all();
  const records = await db
    .prepare('SELECT staff_id, date, status FROM staff_attendance WHERE date LIKE ?')
    .all(`${month}%`);
