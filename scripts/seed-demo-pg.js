// One-off demo data seeder for client demos - Postgres version.
// Run `node scripts/pg-migrate.js` first to apply the schema.
// Run with: node scripts/seed-demo-pg.js
const bcrypt = require('bcryptjs');
const db = require('./pg-lib');
...
async function main() {
  ...
  // Ensure the admin account exists (previously bootstrapped by lib/db.ts on
  // first SQLite boot; Postgres schema setup no longer does this at runtime).
  const adminExists = await db.prepare("SELECT id FROM users WHERE username = 'admin'").get();
  if (!adminExists) {
    const adminHash = bcrypt.hashSync('admin123', 10);
    await db
      .prepare("INSERT INTO users (username, password, role, name) VALUES (?, ?, 'management', 'Administrator')")
      .run('admin', adminHash);
  }
  ...
  for (const c of courses) await insCourse.run(c.name, c.fee, c.duration, null);
  ...
  for (const s of staffList) {
    await insStaff.run({ ...s, address: 'Dehradun, Uttarakhand', remarks: null, user_id: null });
  }
  ...
  const teacherUser1 = await insUser.run('teacher1', teacherHash, 'teacher', 'Rohit Kumar', mobile(), 'teacher1@shikshayogi.demo');
  const teacherUser2 = await insUser.run('teacher2', teacherHash, 'teacher', 'Priya Sharma', mobile(), 'teacher2@shikshayogi.demo');
  const guardianUser = await insUser.run('guardian1', guardianHash, 'guardian', 'Ramesh Chandra', mobile(), 'guardian1@shikshayogi.demo');
  ...
  for (let i = 0; i < totalStudents; i++) {
    ...
    if (i === 0) {
      ...
      const su = await insUser.run('student1', studentHash, 'student', name, mobile(), 'student1@shikshayogi.demo');
      userId = su.lastInsertRowid;
      demoStudentUserId = userId;
    }

    const info = await insStudent.run({
      name,
  ...
  if (demoStudentUserId) {
    await db
      .prepare('INSERT INTO student_guardians (student_id, guardian_user_id) VALUES (?, ?)')
      .run(studentIds[0].id, guardianUser.lastInsertRowid);
  }

  // Link teachers to batches
  await db
    .prepare('INSERT INTO teacher_batches (teacher_user_id, batch_id) VALUES (?, ?)')
    .run(teacherUser1.lastInsertRowid, batchIds[0].id);
  await db
    .prepare('INSERT INTO teacher_batches (teacher_user_id, batch_id) VALUES (?, ?)')
    .run(teacherUser1.lastInsertRowid, batchIds[1].id);
  await db
    .prepare('INSERT INTO teacher_batches (teacher_user_id, batch_id) VALUES (?, ?)')
    .run(teacherUser2.lastInsertRowid, batchIds[2].id);
  ...
  await insNotice.run('Independence Day Holiday', 'The institute will remain closed on 15th August for Independence Day.', 'All', null, null);
  await insNotice.run('Fee Submission Reminder', 'Students with pending dues are requested to clear fees before the 10th of this month.', 'All', null, null);
  await insNotice.run('New Batch Starting', 'A new Web Development batch is starting next month. Refer your friends!', 'All', null, null);
  await insNotice.run('Assignment Submission', 'Please submit your pending assignments to your trainer by this Friday.', 'Batch', batchIds[0].id, null);
  await insNotice.run('Parent-Teacher Meet', 'A parent-teacher meet is scheduled for this Saturday at 11 AM.', 'All', null, null);
  console.log('Seeded 5 notices');

  // --- Attendance (last ~45 weekdays for first 2 batches) ---
  const insAttendance = db.prepare(
    'INSERT INTO attendance (batch_id, student_id, date, status, marked_by) VALUES (@batch_id, @student_id, @date, @status, @marked_by) ON CONFLICT (batch_id, student_id, date) DO NOTHING'
  );
  ...
  for (const batch of batchIds.slice(0, 2)) {
    ...
    while (daysMarked < 45 && dayOffset < 70) {
      ...
      for (const s of batchStudents) {
        await insAttendance.run({ batch_id: batch.id, student_id: s.id, date: dateStr, status: rand(ATT_STATUSES), marked_by: null });
      }
      daysMarked++;
    }
  }
  ...
  for (const [idx, bi] of [0, 1, 2].entries()) {
    ...
    for (let day = 0; day < 6; day++) {
      for (const [si, st] of startHours.entries()) {
        const subject = subjects[(day + si) % subjects.length];
        const end = String(Number(st.slice(0, 2)) + 1).padStart(2, '0') + ':00';
        await insSlot.run(batch.id, day, st, end, subject, slotTeachers[idx]);
      }
    }
  }
  ...
  const insMark = db.prepare(
    'INSERT INTO exam_marks (exam_id, student_id, marks, remarks) VALUES (?, ?, ?, ?) ON CONFLICT (exam_id, student_id) DO NOTHING'
  );
  const adminRow = await db.prepare("SELECT id FROM users WHERE username = 'admin'").get();
  for (const [idx, bi] of [0, 1, 2].entries()) {
    const batch = batchIds[bi];
    ...
    for (const def of examDefs) {
      const info = await insExam.run(def.name, batch.id, rand(subjects), daysAgo(def.daysBack), 100, slotTeachers[idx]);
      for (const s of batchStudents) {
        if (!def.complete && Math.random() > 0.5) continue; // latest exam partially marked
        const marks = randInt(32, 98);
        await insMark.run(info.lastInsertRowid, s.id, marks, marks < 40 ? 'Needs improvement' : marks > 90 ? 'Excellent' : null);
      }
    }
  }
  ...
  await insLeave.run(studentIds[0].id, demoStudentUserId, daysAgo(12), daysAgo(10), 'Fever and doctor advised rest.', 'Approved', 'Get well soon. Approved.', adminRow.id, now);
  await insLeave.run(studentIds[0].id, guardianUser.lastInsertRowid, daysAgo(-3), daysAgo(-5), 'Family function out of town.', 'Pending', null, null, null);
  await insLeave.run(studentIds[0].id, demoStudentUserId, daysAgo(30), daysAgo(29), 'Not feeling well.', 'Rejected', 'Mock test scheduled that day, please attend.', adminRow.id, now);
  ...
  await insQuery.run(studentIds[0].id, demoStudentUserId, 'Extra classes for Physics', 'Can we get one extra doubt-clearing session for Physics before the mock test?', 'Answered', 'Yes, an extra session is scheduled this Saturday at 10 AM.', adminRow.id, now);
  await insQuery.run(studentIds[0].id, demoStudentUserId, 'Study material for Biology', 'Where can I get the printed notes for the last Biology unit?', 'Open', null, null, null);
  await insQuery.run(studentIds[0].id, guardianUser.lastInsertRowid, 'Fee installment date', 'Can the next installment date be moved by a week?', 'Open', null, null, null);
  ...
  for (const s of studentIds) await setFeeType.run('Default', rand(FEE_TYPES), s.id);
  await db.prepare("UPDATE students SET fee_type='Monthly' WHERE id = ?").run(studentIds[0].id);
  await db.exec('UPDATE fees SET fee_type = (SELECT fee_type FROM students s WHERE s.id = fees.student_id)');
  await db.prepare('UPDATE batches SET advance_fee = 1 WHERE id IN (?, ?)').run(batchIds[0].id, batchIds[5].id);
